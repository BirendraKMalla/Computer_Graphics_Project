...
def main():
    ...
    pygame.display.set_caption("2D Balloon Popping - Advanced CG Edition")
    ...
    while running:
        ...
        if state == GameState.DIFFICULTY_SELECT:
            draw_text_outlined(screen, "CHOOSE SKILL LEVEL", font_large, COLOR_WHITE, (WIDTH//2 - 280, 150))
            for i, d in enumerate([Difficulty.EASY, Difficulty.NORMAL, Difficulty.HARD]):
                draw_text_outlined(screen, f"{i+1}: {d.label}", font_medium, d.color, (WIDTH//2 - 100, 300 + i*60))
        elif state == GameState.MENU:
            draw_text_outlined(screen, "BALLOON POPPING: ADVANCED CG", font_large, COLOR_YELLOW, (WIDTH//2 - 400, 100))
            draw_text_outlined(screen, f"Difficulty: {difficulty.label}", font_small, difficulty.color, (WIDTH//2 - 70, 200))
            for i, d in enumerate(DURATIONS):
                color = COLOR_ORANGE if i == selected_dur_idx else COLOR_WHITE
                text = f"> {d}s Mode <" if i == selected_dur_idx else f"{d}s Mode"
                draw_text_outlined(screen, text, font_medium, color, (WIDTH//2 - 90, 280 + i * 50))
            hs_list = high_scores.get(str(DURATIONS[selected_dur_idx]), [0])
            draw_text_outlined(screen, "TOP RECORDS:", font_small, COLOR_GREEN, (WIDTH//2 - 80, 480))
            for i, h in enumerate(hs_list): draw_text_outlined(screen, f"{i+1}. {h}", font_small, COLOR_WHITE, (WIDTH//2 - 40, 510 + i * 30))
            draw_text_outlined(screen, "SPACE to Enter Session | BACKSPACE for Difficulty", font_small, COLOR_WHITE, (WIDTH//2 - 250, 650))
        elif state == GameState.PLAYING and session:
            ...
            draw_text_outlined(screen, "SESSION COMPLETE", font_large, COLOR_RED, (WIDTH//2 - 250, 100))
            draw_text_outlined(screen, f"Point Total: {s.score}", font_medium, COLOR_WHITE, (WIDTH//2 - 100, 200))
            acc = (s.stat_hit / s.stat_fired * 100) if s.stat_fired > 0 else 0
            draw_text_outlined(screen, f"Accuracy: {acc:.1f}%", font_small, COLOR_CYAN, (WIDTH//2 - 80, 280))
            draw_text_outlined(screen, f"Small Pops: {s.stat_small}", font_small, COLOR_RED, (WIDTH//2 - 80, 320))
            draw_text_outlined(screen, f"Large Pops: {s.stat_large}", font_small, COLOR_BLUE, (WIDTH//2 - 80, 360))
            cur_dur = s.duration; hs_list = high_scores.get(str(cur_dur), [0])
            is_new = s.score >= hs_list[0]
            hs_text = "NEW HIGH SCORE!" if is_new else f"Best: {hs_list[0]}"
            hs_color = (abs(int(math.sin(time.time()*12)*100)) + 155, 255, 155) if is_new else COLOR_GREEN
            draw_text_outlined(screen, hs_text, font_medium, hs_color, (WIDTH//2 - 130, 420))
            draw_text_outlined(screen, "SPACE to Replay | ESC for Main Menu", font_small, COLOR_WHITE, (WIDTH//2 - 200, 550))
